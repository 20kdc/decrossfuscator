...
// map/mapBack/mapName will be setup on first reloadObf call
rapture.reloadObf = function() {
 rapture.map = new Map();
 rapture.mapBack = new Map();
 ...
 if (!ig.System) {
  ...
  var checkMap = function () {
   ...
   var tsSym = rapture.map.get("TileSheet") || "TileSheet";
   var cfjSym = rapture.map.get("createFromJson") || "createFromJson";
   if (ig[databaseSym])
   ...
   for (var i = 0; i < maps.length; i++) {
    try {
     ...
     for (var j = 0; j < mapping.length; j += 3) {
      ...
      rapture.mapBack.set(mapping[j + 2][1], mapping[j][1]);
     }
     if (checkMap())
      break;
    } catch (e) {
...
// NOT PART OF API. This function deletes itself before anything else can call it again.
rapture.loadMods = function () {
 ...
 for (var i = 0; i < strs.length; i++) {
  ...
  try {
   ...
   if (!(("disable-" + strs[i]) in rapture.config))
 ...
 // Dependencies ARE NOW strict (to prevent crashes). Circular loops are still undefined behavior.
 for (var pass = 0; pass < modsToEnable.length; pass++) {
  ...
  for (var i = 0; i < modsToEnable.length; i++) {
   ...
   for (var dep = 0; dep < header.dependencies.length; dep++) {
    var depId = header.dependencies[dep];
    var doesNotMatter = false;
    if (depId[0] == "?") {
     depId = depId.substring(1);
     doesNotMatter = true;
    }
    var depIdx = modsToEnable.indexOf(depId);
    if (depIdx > i) {
     didPushback = true;
     // Swap
     ...
     break;
    } else if ((depIdx == -1) && (!doesNotMatter)) {
     if ((rapture.knownMods.indexOf(depId) == -1) || (modsWithFailedDeps.indexOf(depId) != -1)) {
      // Cannot safely enable mod! Get rid of it.
      modsWithFailedDeps.push(modsToEnable[i]);
      modsToEnable.splice(i, 1);
      pass--;
      didPushback = true;
      break;
     } else {
      // The mod is enabled, and the pass *restarted*
      pass--;
      didPushback = true;
      modsToEnable.push(depId);
      break;
     }
    }
   }
 ...
 // Dependencies sorted; start enabling mods.
 rapture.enabledMods = [];
 rapture.enabledModCallbacks = [];
 for (var i = 0; i < modsToEnable.length; i++) {
