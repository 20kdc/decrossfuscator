...
function Snippet(namesU, namesO, snippetArray) {
 ...
 this.namesU = namesU;
 this.namesO = namesO;
 this.dependencyCheck = function (map, log) {
   ...
   if (tkn[0] == "id") {
    ...
    if (tkn[1].startsWith("$$")) {
     if (!map.has(tkn[1].substring(2))) {
      if (log)
       console.error(this.displayName + " unable to resolve " + tkn[1]);
      return false;
     }
    }
   }
 ...
 this.calculate = function (map, worldTokens) {
  ...
  for (var i = 0; i < worldTokens.length; i++) {
   ...
   if (r) {
    if (matchPos != null) {
     console.error(this.displayName + " got multiple matches, failing");
     return null;
    }
    matchPos = i;
   }
  ...
  if (matchPos != null) {
   ...
   for (var j = 0; j < this.tokens.length; j++) {
    if (this.tokens[j][0] == "id") {
     ...
     if (nameAssign != -1)
      map.set(this.namesU[nameAssign], worldTokens[matchPos + j][1]);
    }
   }
   return matchPos;
  }
...
function Matcher(ccVersion) {
 this.snippets = [];
 this.deleteThese = [];
 this.lostSymbols = [];
 this.ccVersionAEFunction = ccver.getCCVersionAE(ccVersion);
 ...
 this.loadProfile = function (profile) {
  ...
  for (var k in profile.lostSymbols)
   if (this.ccVersionAEFunction(k))
    this.lostSymbols = this.lostSymbols.concat(profile.lostSymbols[k]);
 };
...
function MatcherResults(snippetsConv, tokens, m) {
 ...
 while (snippetOrder.length > 0) {
  ...
  for (var i = 0; i < snippetOrder.length; i++) {
   ...
   if (snippet.dependencyCheck(this.map, false)) {
  ...
  if (!didWeDoAnything)
   if (snippetOrder.length > 0) {
    var snippet = snippetsConv[snippetOrder.shift()];
    snippet.dependencyCheck(this.map, true);
    snippet.calculate(this.map, this.sourceTokens);
   }
 }
 // Used by Rosebud
